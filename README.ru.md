...
Интерфейс по умолчанию английский, русский включается в шторке с ключом — «Язык».
...
Система работает и развёрнута, но **её никто не проверял со стороны.** Криптография
стандартная и построения обычные — это не то же самое, что аудит. Прочитайте
[SECURITY.md](SECURITY.md) до того, как положиться на неё в чём-то серьёзном.
...
| Лента комнаты | Ваш ключ | Личное сообщение |
|---|---|---|
| ![Лента](docs/screenshots/feed.png) | ![Шторка ключа](docs/screenshots/sheet.png) | ![Личное](docs/screenshots/direct.png) |

Имена — карты, розданные по ключу участника, а не выбранные ники. Ссылка-приглашение
на среднем снимке настоящая, но мёртвая: снимки делает одноразовый локальный прогон,
поэтому они и не устаревают.

```bash
SCREENSHOTS=1 npx playwright test --grep @screenshot
```

## Что на самом деле лежит у сервера

```mermaid
sequenceDiagram
    participant A as Браузер отправителя
    participant S as Сервер
    participant B as Браузер получателя
    A->>A: собрать конверт, зашифровать ключом комнаты
    A->>S: шифротекст и выбранный срок
    Note over S: хранит байты и дедлайн.<br/>Ни ключа, ни открытого текста.
    S->>B: те же байты
    B->>B: развернуть ключ комнаты из IndexedDB, расшифровать
```

Ключ комнаты не проходит через сервер открытым. У каждого участника он лежит обёрнутым
под его X25519, а в приглашении едет ещё меньше:

```mermaid
flowchart LR
    I["Браузер приглашающего"] -->|"SHA-256 токена,<br/>ключ комнаты под<br/>эфемерным публичным"| S[("Сервер")]
    I -->|"фрагмент ссылки: токен<br/>и эфемерный приватный ключ"| G["Гость"]
    G -->|"сам токен"| S
    S -->|"обёрнутый ключ комнаты"| G
    G -->|"переобёрнутый под свой ключ"| S
```

Фрагмент до сервера не доходит, поэтому токена нет ни в его логах, ни в `Referer`.
На сервере лежит только хэш. С погашением приглашения обёртка удаляется вместе с ним —
пересланная ссылка перестаёт быть ключом от комнаты, чего не дало бы решение положить
ключ комнаты прямо во фрагмент.
...
| `app.challenge.per-ip-limit` | `30` | Вызовов на вход с одного адреса в минуту |
| `app.challenge.max-live` | `10000` | Потолок на число невыбранных вызовов |
| `app.challenge.max-addresses` | `20000` | Потолок на число адресов, посчитанных за окно |
...
Это утверждение тоже проверяемо. CI собирает jar дважды на чистой машине с выключенным
кэшем Gradle — восстановленный артефакт сделал бы сборки одинаковыми, ничего не доказав, —
и сравнивает файлы побайтно: [`.github/workflows/ci.yml`](.github/workflows/ci.yml).
Те же две проверки выполняются локально, нужны только JDK 21 и этот репозиторий:
...
Подробности решений и обоснования — в [CLAUDE.md](CLAUDE.md).
...
Разовый прогон на хостинге без установки сервиса и автозагрузки — [deploy/TESTRUN.md](deploy/TESTRUN.md)
(на английском).
...
Playwright закрывает то, до чего Java не достаёт: камеру при переносе ключа по QR, обмен
ключами между двумя разными личностями и переключение языка. Прогон сам поднимает
приложение на отдельном порту с пустой базой — ссылка владельца печатается только при
первом запуске.

npm заведён только ради этих тестов. В раздаваемый код из него не попадает ничего:
libsodium по-прежнему вендорится без сборщика.

## Уязвимости

Приватным сообщением через вкладку Security этого репозитория. Публичную задачу на то,
что затрагивает конфиденциальность, не заводите. Подробности и список известных ограничений —
...
AGPL, а не разрешительная лицензия, выбрана намеренно. Доверие к веб-мессенджеру держится
на том, что раздаваемый код совпадает с опубликованным исходником — ради этой проверки при
каждом старте печатается отпечаток бандла. Лицензия, разрешающая крутить для чужих людей
изменённый закрытый сервер, обесценила бы её.

Вендоренный сторонний код сохраняет свои лицензии, они перечислены в
[PROVENANCE.md](src/main/resources/static/vendor/PROVENANCE.md).
