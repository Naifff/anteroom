...
| `app.max-file-bytes` | `22020096` | Потолок размера файла (шифротекст) |
| `app.room-quota-bytes` | `2147483648` | Квота на комнату |
| `app.instance-quota-bytes` | `21474836480` | Квота на весь сервер |
...
Файл живёт по тому же сроку, что и сообщение, и удаляется вместе с ним.
...
«Прикрепить» — «Одноразовая ссылка». Содержимое откроется у первого, кто нажмёт кнопку, и
будет уничтожено на сервере в тот же момент.
